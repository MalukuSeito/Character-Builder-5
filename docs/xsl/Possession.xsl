...
<xsl:if test="Weight>-1">
	...
	<div class="Info">
		...
		<p style="text-transform:lowercase"><xsl:if test="Rarity != 'None'" ><xsl:if test="Rarity = 'VeryRare'">very rare</xsl:if><xsl:if test="Rarity != 'VeryRare'"><xsl:value-of select="Rarity"/></xsl:if></xsl:if></p>
	</div>
