...
namespace CB_5e.ViewModels.Character
{
    public class LoadingProgress : BaseDataObject
    {
        ...
        public async Task Load(CancellationToken token = default(CancellationToken))
        {
            ...
            Percentage = cur++ / count;
            token.ThrowIfCancellationRequested();
            await Context.LoadLevelAsync(new FileInfo(Path.Combine(PCLSourceManager.Data.FullName, config.Levels))).ConfigureAwait(false);

            Text = "AbilityScores";
            Percentage = cur++ / count;
            token.ThrowIfCancellationRequested();
            await Context.LoadAbilityScoresAsync(new FileInfo(Path.Combine(PCLSourceManager.Data.FullName, config.AbilityScores))).ConfigureAwait(false);
