            ...
            else if (Context.Player != null)
            {
                ...
                try
                {
                    if (p.FilePath == null)
                    {
                        ...
                        if (!characters.Exists) characters.Create();
                        FileInfo pf = new FileInfo(Path.Combine(characters.FullName, p.Name + ".cb5"));
                        int i = 0;
                        while (pf.Exists) pf = new FileInfo(Path.Combine(characters.FullName, p.Name + " (" + ++i + ").cb5"));
                        p.FilePath = pf;
                    }
