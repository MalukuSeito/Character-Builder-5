...
namespace CB_5e.ViewModels.Modify
{
    public class SettingsEditModel : BaseViewModel, IEditModel
    {
        ...
        {
            using (Stream fs = new FileStream(Path.Combine(PCLSourceManager.Data.FullName, "Config.xml"), FileMode.OpenOrCreate))
            {
                await Task.Run(() => ConfigManager.Serializer.Serialize(fs, Model)).ConfigureAwait(false);
