...
namespace CB_5e.ViewModels.Modify
{
    public class LevelEditModel : BaseViewModel, IEditModel
    {
        ...
        {
            using (Stream fs = new FileStream(Path.Combine(PCLSourceManager.Data.FullName, Context.Config.Levels), FileMode.OpenOrCreate))
            {
                await Task.Run(() => Level.Serializer.Serialize(fs, Model)).ConfigureAwait(false);
