...
namespace Character_Builder_5
{
	public partial class Form1 : Form
	{
		...
		public Form1()
		{
			...
			RarityBox.Items.Clear();
			foreach (Rarity s in Enum.GetValues(typeof(Rarity))) RarityBox.Items.Add(s);
			RarityBox.Items.Add("(default)");
			...
			if (listItems.SelectedItem != null)
			{
				...
				if (listItems.SelectedItem is MagicProperty)
				{
					...
					if (mp.Base == null || mp.Base == "")
					{
						...
						addToJournalButton.Enabled = true;
					}
					else
					{
						addToJournalButton.Enabled = false;
						if (inventory2.SelectedItem != null && ((Possession)inventory2.SelectedItem).BaseItem != null && ((Possession)inventory2.SelectedItem).BaseItem != "")
				...
				if (listItems.SelectedItem is Spell)
				{
					...
					if (spellbookFeaturesBox.SelectedItem != null && spellbookFeaturesBox.SelectedItem is SpellcastingCapsule)
						addspellbookButton.Enabled =
							(Utils.Matches(Program.Context, ((Spell)listItems.SelectedItem), ((SpellcastingCapsule)spellbookFeaturesBox.SelectedItem).Spellcastingfeature.PrepareableSpells, ((SpellcastingCapsule)spellbookFeaturesBox.SelectedItem).Spellcastingfeature.SpellcastingID)
								 || Program.Context.Player.GetSpellcasting(((SpellcastingCapsule)spellbookFeaturesBox.SelectedItem).Spellcastingfeature.SpellcastingID).CanBeAdded((Spell)listItems.SelectedItem, Program.Context.Player, Program.Context))
							&& !Program.Context.Player.GetSpellcasting(((SpellcastingCapsule)spellbookFeaturesBox.SelectedItem).Spellcastingfeature.SpellcastingID).GetSpellbook(Program.Context.Player, Program.Context).Contains((Spell)listItems.SelectedItem);
				}
		...
		private void itemCategories_SelectedIndexChanged(object sender, EventArgs e)
		{
			if (itemCategories.SelectedItem != null)
			{
				...
				if (itemCategories.SelectedItem is Category)
				{
					...
					ItemCounter.Value = 1;
					actionBox.Controls.Clear();
					actionBox.Controls.Add(addToJournalButton);
		...
		public void UpdateJournal(bool updateside = true)
		{
			...
			int consumable = 0;
			int uncommonplus = 0;
			foreach (Possession p in Program.Context.Player.GetItemsAndPossessions(false, true))
			{
				if (p.Consumable) consumable++;
				else if (p.Rarity == Rarity.Common) common++;
				else if (p.Rarity >= Rarity.Uncommon) uncommonplus++;
			}
			string s = "Items: ";
			if (uncommonplus > 0) s = s + uncommonplus + " Uncommon+ ";
			if (common > 0) s = s + common + " Common ";
			if (consumable > 0) s = s + consumable + " Consumable";
			journalItemStats.Text = s;
			removeJournalItem.Enabled = false;
		...
		public void UpdateEquipmentLayout(bool updateside = true)
		{
			try
			{
				...
				inventory.Items.AddRange(Program.Context.Player.Boons.Select(b => Program.Context.GetBoon(b, null)).ToArray());
				inventory.Items.AddRange(Program.Context.Player.GetJournalBoons().ToArray());
		...
		public void UpdateInventoryOptions()
		{
			try
			{
				...
				if (!layouting) layouting = true;
				equiptab.SuspendLayout();
				magicpropfeats.Controls.Clear();
				List<Control> magict = new List<Control>();
				...
				if (inventory.SelectedItem is Possession p)
				{
					...
					consumableCheckbox.Enabled = true;
					RarityBox.Enabled = true;
					RarityBox.SelectedItem = p.Rarity;
					consumableCheckbox.Checked = p.Consumable;
					List<Feature> features = p.Collect(level, p, Program.Context, true, true, true).ToList();
					foreach (Feature f in Program.Context.Plugins.FilterPossessionFeatures(features, level, Program.Context.Player, Program.Context))
					{
						ControlAdder.AddControl(magict, p, level, f);
					}
				}
				else if (inventory.SelectedItem is JournalBoon jb)
				{
					...
					updateposs.Enabled = true;
					consumableCheckbox.Enabled = false;
					RarityBox.Enabled = false;
					RarityBox.SelectedItem = Rarity.None;
					consumableCheckbox.CheckState = CheckState.Indeterminate;
					List<Feature> features = Program.Context.GetBoon(jb.Name, null).Collect(level, jb, Program.Context);
					foreach (Feature f in Program.Context.Plugins.FilterBoons(features, level, Program.Context.Player, Program.Context))
					{
						ControlAdder.AddControl(magict, jb, level, f);
					}
				}
				else if (inventory.SelectedItem is Feature f)
				{
					possequip.SelectedIndex = -1;
					possequip.Enabled = false;
					attunedcheck.Enabled = false;
					...
					RarityBox.SelectedItem = Rarity.None;
					consumableCheckbox.CheckState = CheckState.Indeterminate;
					List<Feature> features = f.Collect(level, Program.Context.Player, Program.Context);
					foreach (Feature ff in Program.Context.Plugins.FilterBoons(features, level, Program.Context.Player, Program.Context))
					{
						ControlAdder.AddControl(magict, Program.Context.Player, level, ff);
					}
				}
				else
				{
				...
				magict.Reverse();
				foreach (Control c in magict) magicpropfeats.Controls.Add(c);
				equiptab.ResumeLayout();
				if (!waslayouting) layouting = false;
			}
			catch (Exception e)
		...
		public void UpdateInPlayInner()
		{
			try
			{
				...
				{
					var ais = Program.Context.Player.GetAttack(p, 0, null, null, null, false);
					if (ais != null)
					{
		...
		public void UpdateBackgroundLayout(bool updateside = true)
		{
			try
			{
				...
				else
				{
					...
					ControlAdder.AddControls(back, backt, Program.Context.Player, level);
					foreach (Feature f in Program.Context.Player.GetOnlyBackgroundFeatures(0, true).OrderBy(a => a.Level))
					{
						ControlAdder.AddControl(backt, Program.Context.Player, level, f);
					}
				}
		...
		public void UpdateSpellcastingOuter(bool updateside = true)
		{
			try
			{
				...
				List<Feature> spellfeatures = new List<Feature>(from f in Program.Context.Player.GetFeatures() where f is SpellcastingFeature || f is SpellChoiceFeature || f is ModifySpellChoiceFeature || f is IncreaseSpellChoiceAmountFeature || f is BonusSpellPrepareFeature select f);
				List<SpellcastingFeature> spellcasts = new List<SpellcastingFeature>();
				foreach (Feature f in spellfeatures) if (f is SpellcastingFeature) spellcasts.Add((SpellcastingFeature)f);
		...
		private void deselect_Spell(object sender, EventArgs e)
		{
			...
			if (lb.SelectedItem != null && lb.SelectedItem is Spell && tab != null)
			{
				...
				if (choicebox != null && choicebox.SelectedItem != null && choicebox.SelectedItem is SpellChoiceCapsule && ((SpellChoiceCapsule)choicebox.SelectedItem).Spellchoicefeature != null)
				{
					...
					Program.Context.Player.ModifiedSpellChoice(tab.Name, scf.UniqueID);
			...
			if (lb.SelectedItem != null)
			{
				...
				if (sf != null)
				{
					...
					if (sc.GetPreparedList(Program.Context.Player, Program.Context).Count < Utils.AvailableToPrepare(Program.Context, sf, Program.Context.Player.GetClassLevel(sf.SpellcastingID)))
					{
						...
						sc.ModifiedPreparedList(Program.Context.Player.GetLevel());
					}
					UpdateSpellcastingInner(true, spellfeatures);
					UpdateFormsCompanions();
					UpdateInPlayInner();
		...
		public void UpdateSpellcastingInner(bool updateside = true, List<Feature> spellfeatures = null)
		{
			...
			{
				layouting = true;
				if (spellfeatures == null) spellfeatures = new List<Feature>(from f in Program.Context.Player.GetFeatures() where f is SpellcastingFeature || f is SpellChoiceFeature || f is ModifySpellChoiceFeature || f is IncreaseSpellChoiceAmountFeature || f is BonusSpellPrepareFeature select f);
				List<SpellcastingFeature> spellcasts = new List<SpellcastingFeature>();
				foreach (Feature f in spellfeatures) if (f is SpellcastingFeature) spellcasts.Add((SpellcastingFeature)f);
				if (updateside) UpdateSideLayout();
				foreach (SpellcastingFeature sf in spellcasts)
					...
					if (sf != null)
					{
						if (scf != null)
						{
							...
							foreach (Feature f in spellfeatures)
							{
								...
								else if (f is BonusSpellPrepareFeature bspf && bspf.Matches(sf.SpellcastingID) && bspf.AddTo == PreparationMode.ClassList)
								{
									foreach (string s in bspf.Spells)
									{
										available.Add(new ModifiedSpell(Program.Context.GetSpell(s, bspf.Source), bspf.KeywordsToAdd, false, false));
									}
									available.AddRange(Utils.FilterSpell(Program.Context, bspf.Condition, sf.SpellcastingID).Select(s => new ModifiedSpell(s, bspf.KeywordsToAdd, false, false)));
								}
								else if (f is IncreaseSpellChoiceAmountFeature && ((IncreaseSpellChoiceAmountFeature)f).UniqueID == scf.UniqueID) amount += ((IncreaseSpellChoiceAmountFeature)f).Amount;
							}
							available.Sort();
							chosen.Sort();
							Control tab = null;
		...
		public void UpdateClass(bool updateside = true)
		{
			try
			{
				...
				{
					ControlAdder.AddControl(classt, Program.Context.Player, level, f);
				}
				classt.Reverse();
		...
		public void UpdateRaceLayout(bool updateside = true)
		{
			try
			{
				...
				else
				{
					...
					ControlAdder.AddControls(rac, racet, Program.Context.Player, level);
					foreach (Feature f in Program.Context.Player.GetRaceFeatures(0, true).OrderBy(a => a.Level))
					{
						ControlAdder.AddControl(racet, Program.Context.Player, level, f);
					}
				}
		...
		{
			ChoiceList choicer = (ChoiceList)sender;
			if (choicer != null && choicer.SelectedItem != null)
			{
				Program.Context.MakeHistory("");
				if (choicer.ChoiceProvider is JournalPossession jp)
				{
					JournalEntry je;
					if (journalEntries.SelectedItem is JournalEntry j)
					{
						je = j;
					}
					else
					{
						...
						Program.Context.Player.ComplexJournal.Add(je);
					}
					if (!je.Possessions.Contains(jp))
					{
						choicer.ChoiceProvider = jp = new JournalPossession(jp);
						je.Possessions.Add(jp);
					}
				}
				else if (choicer.ChoiceProvider is JournalBoon jb)
				{
					JournalEntry je;
					if (journalEntries.SelectedItem is JournalEntry j)
					{
					...
					if (!je.Boons.Contains(jb))
					{
						choicer.ChoiceProvider = jb = new JournalBoon(jb);
						je.Boons.Add(jb);
					}
				}
				Choice c = choicer.ChoiceProvider.GetChoice(choicer.Name);
				if (c == null || c.Value == "")
				{
					bool old = ConfigManager.AlwaysShowSource;
					ConfigManager.AlwaysShowSource = true;
					if (choicer.SelectedIndex == choicer.Items.Count - 1) choicer.ChoiceProvider.SetChoice(choicer.Name, Interaction.InputBox("Custom Entry:", "CB 5"));
					else choicer.ChoiceProvider.SetChoice(choicer.Name, choicer.SelectedItem.ToString());
					ConfigManager.AlwaysShowSource = old;
				}
				else choicer.ChoiceProvider.RemoveChoice(choicer.Name);
		...
		public void Choice_DoubleClick(object sender, EventArgs e)
		{
			...
			if (choicer != null && choicer.SelectedItem != null)
			{
				...
				bool old = ConfigManager.AlwaysShowSource;
				ConfigManager.AlwaysShowSource = true;
				if (c == null || c.Value == "") choicer.ChoiceProvider.SetChoice(choicer.Name, choicer.SelectedItem.ToString());
				else choicer.ChoiceProvider.RemoveChoice(choicer.Name);
				ConfigManager.AlwaysShowSource = old;
				Program.MainWindow.UpdateLayout();
			}
		...
		private void addspellbookButton_Click(object sender, EventArgs e)
		{
			if (spellbookFeaturesBox.SelectedItem != null && listItems.SelectedItem != null && listItems.SelectedItem is Spell)
			{
				...
				var scc = Program.Context.Player.GetSpellcasting(sc.SpellcastingID);
				scc.GetAdditionalList(Program.Context.Player, Program.Context).Add(((Spell)listItems.SelectedItem).Name + " " + ConfigManager.SourceSeperator + " " + ((Spell)listItems.SelectedItem).Source);
				scc.ModifiedAdditionalList(Program.Context.Player.GetLevel());
				UpdateLayout();
			}
		}
		...
		{
			if (inventory2.SelectedItem != null && listItems.SelectedItem != null && listItems.SelectedItem is MagicProperty mp)
			{
				Program.Context.MakeHistory("");
				Possession p = (Possession)inventory2.SelectedItem;
				if (p is JournalPossession jp)
				{
					JournalEntry je;
					if (journalEntries.SelectedItem is JournalEntry j)
					{
					...
					if (!je.Possessions.Contains(jp))
					{
						jp = new JournalPossession(jp);
						je.Possessions.Add(jp);
					}
					jp.MagicProperties.Add(mp.Name + " " + ConfigManager.SourceSeperator + " " + mp.Source);
					UpdateLayout();
					journalEntries.SelectedItem = je;
				}
				else
				{
					int stack = 1;
					...
					{
						p.Count -= stack;
						p = new Possession(p, mp);
					}
					else
					{
						p.MagicProperties.Add(mp.Name + " " + ConfigManager.SourceSeperator + " " + mp.Source);
					}
		...
		private void inventory_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			else if (journalPossessions.SelectedItem is JournalBoon jb)
			{
				displayElement.Navigate("about:blank");
				displayElement.Document.OpenNew(true);
				displayElement.Document.Write(new MultiFeature(jb.DisplayName, jb.DisplayText, Program.Context.GetBoon(jb.Name, null)).ToHTML());
				displayElement.Refresh();
			}
			UpdateInventoryOptions();
		}
		...
		{
			if (inventory.SelectedItem is Possession p && magicproperties.SelectedItem != null)
			{
				Program.Context.MakeHistory("");
				if (inventory.SelectedItem is JournalPossession jp)
				{
					JournalEntry je;
					if (journalEntries.SelectedItem is JournalEntry j)
					{
		...
		private void removepossesion_Click(object sender, EventArgs e)
		{
			...
			else if (inventory.SelectedItem is Possession)
			{
				Program.Context.MakeHistory("");
		...
		private void updateposs_Click(object sender, EventArgs e)
		{
			...
			{
				Program.Context.MakeHistory("");
				jb.DisplayName = possname.Text;
				jb.DisplayText = possdescription.Text;
				UpdateLayout();
			}
		}
		...
		private void possequip_SelectedIndexChanged(object sender, EventArgs e)
		{
			if (!layouting)
			{
				if (inventory.SelectedItem is Possession)
				{
					...
					foreach (Possession pos in Program.Context.Player.GetItemsAndPossessions(includeBanked: true, journalOnly: true))
					{
						if (string.Equals(pos.Equipped, es, StringComparison.InvariantCultureIgnoreCase)) pos.Equipped = EquipSlot.None;
					}
					Possession p = (Possession)inventory.SelectedItem;
		...
		private void posscharges_ValueChanged(object sender, EventArgs e)
		{
			if (!layouting)
			{
				if (inventory.SelectedItem is JournalPossession jp)
				{
					...
					jp.ChargesUsed = (int)posscharges.Value;
					UpdateLayout();
				}
				else if (inventory.SelectedItem is Possession)
				{
		...
		private void journalEntries_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (journalEntries.SelectedItem is JournalEntry)
			{
				...
				journalNotesBox.SelectedIndex = -1;
				journalNoteAddButton.Enabled = true;
			...
			else
			{
				...
				removeJournalItem.Enabled = false;
				journalNoteAddButton.Enabled = false;
				journalNotesRemoveButton.Enabled = false;
				journalNotesText.Enabled = false;
				journalNotesText.Text = "";
			}
			UpdateJournalItems();
			UpdateJournalNotes();
			if (!waslayouting) layouting = false;
		...
		private void UpdateJournalItems()
		{
			bool waslayouting = layouting;
			if (!layouting) layouting = true;
			journalPossessions.Items.Clear();
			if (journalEntries.SelectedItem is JournalEntry je)
			{
				if (showAllItems.Checked)
				{
					Dictionary<Guid, JournalPossession> possesions = new Dictionary<Guid, JournalPossession>();
					Dictionary<Guid, JournalBoon> boons = new Dictionary<Guid, JournalBoon>();
					foreach (JournalEntry j in Program.Context.Player.ComplexJournal)
					{
						foreach (JournalPossession p in j.Possessions)
						{
							if (p.Deleted && j != je) possesions.Remove(p.Guid);
							else if (possesions.ContainsKey(p.Guid)) possesions[p.Guid] = p;
							else possesions.Add(p.Guid, p);
						}
						foreach (JournalBoon b in j.Boons)
						{
							if (b.Deleted && j != je) boons.Remove(b.Guid);
							else if (boons.ContainsKey(b.Guid)) boons[b.Guid] = b;
							else boons.Add(b.Guid, b);
						}
						if (j == je) break;
					}
					foreach (JournalPossession jp in possesions.Values) journalPossessions.Items.Add(jp, !jp.Banked);
					foreach (JournalBoon jb in boons.Values) journalPossessions.Items.Add(jb, !jb.Banked);

				}
				else
				{
					foreach (JournalPossession jp in je.Possessions) journalPossessions.Items.Add(jp, !jp.Banked);
					foreach (JournalBoon jb in je.Boons) journalPossessions.Items.Add(jb, !jb.Banked);
				}
			}
			if (!waslayouting) layouting = false;
		}
		...
		private void exportXMLToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			{
				if (Properties.Settings.Default.LastPDFFolder == null || Properties.Settings.Default.LastPDFFolder == "") od.InitialDirectory = Path.GetDirectoryName(lastfile);
				od.FileName = Path.GetFileNameWithoutExtension(lastfile) + ".json";
			}
			od.Filter = "JSON|*.json";
			od.Title = "Save a JSON File";
			if (od.ShowDialog() == DialogResult.OK && od.FileName != "")
			{
				Properties.Settings.Default.LastPDFFolder = Path.GetDirectoryName(od.FileName);
				try
				{
					using (var fs = od.OpenFile())
					{
						...
						//serializer.Serialize(fs, new ExportPlayer(Program.Context.Player));
						JsonSerializer.Serialize(fs, new ExportPlayer(Program.Context.Player), new JsonSerializerOptions()
						{
							WriteIndented = true,
				...
				{
					System.Windows.Forms.MessageBox.Show(ex.Message + "\n" + ex.InnerException + "\n" + ex.StackTrace, "Error exporting XML to " + od.FileName);
				}
			}
		}
		...
		{
			UpdateJournalItems();
			removeJournalItem.Text = "Remove " + (showAllItems.Checked ? "Item" : "Entry");
		}

		private void addToJournalButton_Click(object sender, EventArgs e)
		{
			if (listItems.SelectedItem != null)
			{
				Program.Context.MakeHistory("");
				...
				if (listItems.SelectedItem is Item i)
				{
					int count = (int)ItemCounter.Value;
					je.Possessions.Add(new JournalPossession(new Possession(Program.Context, i, count)));
				}
				if (listItems.SelectedItem is MagicProperty && (((MagicProperty)listItems.SelectedItem).Base == null || ((MagicProperty)listItems.SelectedItem).Base == ""))
					je.Possessions.Add(new JournalPossession(new Possession(Program.Context, (Item)null, (MagicProperty)listItems.SelectedItem)));
				if (listItems.SelectedItem is Feature)
				{
					je.Boons.Add(new JournalBoon(((Feature)listItems.SelectedItem).Name + " " + ConfigManager.SourceSeperator + " " + ((Feature)listItems.SelectedItem).Source));
				}
				if (listItems.SelectedItem is Spell sp)
				{
					je.Possessions.Add(new JournalPossession(new Possession(Program.Context, sp.Name + " " + ConfigManager.SourceSeperator + " " + sp.Source, 1)));
				}
				UpdateLayout();
				journalEntries.SelectedItem = je;
			}
		...
		private void journalPossessions_SelectedIndexChanged(object sender, EventArgs e)
		{
			removeJournalItem.Enabled = journalPossessions.SelectedItem != null;
			if (journalPossessions.SelectedItem is JournalPossession jp)
			{
				displayElement.Navigate("about:blank");
				displayElement.Document.OpenNew(true);
				displayElement.Document.Write(new DisplayPossession(jp, Program.Context.Player).ToHTML());
				displayElement.Refresh();
			}
			else if (journalPossessions.SelectedItem is JournalBoon jb)
			{
		...
		private void removeJournalItem_Click(object sender, EventArgs e)
		{
			if (showAllItems.Checked)
			{
				if (journalPossessions.SelectedItem is JournalPossession jp)
		...
		private void journalPossessions_ItemCheck(object sender, ItemCheckEventArgs e)
		{
			if (layouting) return;
			var item = journalPossessions.SelectedItem;
			if (journalPossessions.Items[e.Index] is JournalPossession jp)
			{
				JournalEntry je;
				if (journalEntries.SelectedItem is JournalEntry j)
				{
				...
				jp.Banked = e.NewValue == CheckState.Unchecked;
			}
			else if (journalPossessions.Items[e.Index] is JournalBoon jb)
			{
				JournalEntry je;
				if (journalEntries.SelectedItem is JournalEntry j)
				{
				...
				jb.Banked = e.NewValue == CheckState.Unchecked;
			}
			UpdateLayout();
			journalPossessions.SelectedItem = item;
		}
		...
		private void button1_Click_3(object sender, EventArgs e)
		{
			...
			je.Possessions.Add(new JournalPossession(p));
			UpdateLayout();
		}

		private bool textUpdate = false;

		private void journalNotesText_TextChanged(object sender, EventArgs e)
		{
			if (layouting) return;
			Program.Context.MakeHistory("JournalNotesText");
			if (journalEntries.SelectedItem is JournalEntry j)
			{
				if (journalNotesBox.SelectedIndex >= 0 && journalNotesBox.SelectedIndex < j.Notes.Count)
				{
					j.Notes[journalNotesBox.SelectedIndex] = journalNotesText.Text;
					textUpdate = true;
					journalNotesBox.Items[journalNotesBox.SelectedIndex] = journalNotesText.Text.IndexOfAny(new char[] { '\r', '\n' }) == -1 ? journalNotesText.Text : journalNotesText.Text.Substring(0, journalNotesText.Text.IndexOfAny(new char[] { '\r', '\n' }));
					textUpdate = false;
				}
			}
			//UpdateJournalNotes();
		...
		private void journalNotesBox_SelectedIndexChanged(object sender, EventArgs e)
		{
			if (textUpdate) return;
			bool waslayouting = layouting;
			if (!layouting) layouting = true;
			if (journalEntries.SelectedItem is JournalEntry j)
			{
				if (journalNotesBox.SelectedIndex >= 0 && journalNotesBox.SelectedIndex < j.Notes.Count)
				{
					journalNotesText.Text = j.Notes[journalNotesBox.SelectedIndex];
					if (!journalNotesText.Enabled) journalNotesText.Enabled = true;
					journalNotesRemoveButton.Enabled = true;
				}
				else
				{
					journalNotesText.Text = "";
		...
		private void UpdateJournalNotes()
		{
			bool waslayouting = layouting;
			if (!layouting) layouting = true;
			int index = journalNotesBox.SelectedIndex;
			journalNotesBox.Items.Clear();
			if (journalEntries.SelectedItem is JournalEntry j)
			{
				journalNotesBox.Items.AddRange((from s in j.Notes select s.IndexOfAny(new char[] { '\r', '\n' }) == -1 ? s : s.Substring(0, s.IndexOfAny(new char[] { '\r', '\n' }))).ToArray());
				if (index < j.Notes.Count) journalNotesBox.SelectedIndex = index;
			}
			if (!waslayouting) layouting = false;
		}

		private void journalNoteAddButton_Click(object sender, EventArgs e)
		{
			if (journalEntries.SelectedItem is JournalEntry j)
			{
				Program.Context.MakeHistory("");
				int index = journalNotesBox.Items.Count;
				j.Notes.Add("New Entry");
				journalNotesBox.Items.Add("New Entry");
				journalNotesBox.SelectedIndex = index;
			}
		}

		private void journalNotesRemoveButton_Click(object sender, EventArgs e)
		{
			if (journalEntries.SelectedItem is JournalEntry j)
			{
				if (journalNotesBox.SelectedIndex >= 0 && journalNotesBox.SelectedIndex < j.Notes.Count)
				{
					Program.Context.MakeHistory("");
					j.Notes.RemoveAt(journalNotesBox.SelectedIndex);
					journalNotesBox.SelectedIndex = -1;
					UpdateJournalNotes();
				}
			}
		...
		private void RarityBox_SelectedIndexChanged(object sender, EventArgs e)
		{
			if (!layouting)
			{
				...
				{
					Program.Context.MakeHistory("");
					if (RarityBox.SelectedItem is Rarity r)
					{
						p.Rarity = r;
					}
					else
					{
						p.RarityOverride = null;
					}
					Program.Context.Player.AddPossession(p);
					UpdateLayout();
				}
		...
		private void consumableCheckbox_CheckedChanged(object sender, EventArgs e)
		{
			if (!layouting)
			{
				if (inventory.SelectedItem is Possession p)
				{
					Program.Context.MakeHistory("");
					CheckState s = consumableCheckbox.CheckState;
					if (consumableCheckbox.CheckState == CheckState.Indeterminate)
					{
						p.ConsumableOverride = null;
					}
					else
					{
						p.ConsumableOverride = consumableCheckbox.Checked;
					}
					Program.Context.Player.AddPossession(p);
					UpdateLayout();
					layouting = true;
					consumableCheckbox.CheckState = s;
					layouting = false;
				}
			}
		}
