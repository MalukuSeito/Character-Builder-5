...
namespace CB_5e.Views.Modify.Collections
{
	...
	public partial class MonsterSkillListPage : ContentPage
	{
        public static XmlSerializer Serializer = new XmlSerializer(typeof(MonsterSkillBonus));
        private List<MonsterSkillBonus> entries;
        public ObservableRangeCollection<MonsterSkillViewModel> Entries { get; set; } = new ObservableRangeCollection<MonsterSkillViewModel>();
        ...
        public MonsterSkillListPage(IEditModel parent, string property, bool modal = true)
        {
            Model = parent;
            Modal = modal;
        ...
        {
            entries = (List<MonsterSkillBonus>)Model.GetType().GetRuntimeProperty(Property).GetValue(Model);
            Fill();
        }
        private void Fill() => Entries.ReplaceRange(entries.Select(f => new MonsterSkillViewModel(f)));
        ...
        private void Paste_Clicked(object sender, EventArgs e)
        {
            try
            {
                ...
                using (StringReader sr = new StringReader(DependencyService.Get<IClipboardService>().GetTextData()))
                {
                    ...
                    if (o is MonsterSkillBonus msb) entries.Add(msb);
                    Fill();
                }
            } catch (Exception ex)
            {
        ...
        {
            Model.MakeHistory();
            MonsterSkillBonus t = new MonsterSkillBonus();
            MonsterSkillViewModel vm = new MonsterSkillViewModel(t);
            entries.Add(t);
            Entries.Add(vm);
            await Navigation.PushAsync(new MonsterSkillEditPage(vm, Model.Context));
        }

        private async void Entries_ItemSelected(object sender, SelectedItemChangedEventArgs e)
        {
            if (e.SelectedItem is MonsterSkillViewModel fvm) {
                if (move >= 0)
                {
                    Model.MakeHistory();
                    foreach (MonsterSkillViewModel ff in Entries) ff.Moving = false;
                ...
                {
                    Model.MakeHistory();
                    await Navigation.PushAsync(new MonsterSkillEditPage(fvm, Model.Context));
                    (sender as ListView).SelectedItem = null;
                }
            }
        }
        ...
        {
            if (((MenuItem)sender).BindingContext is MonsterSkillViewModel f)
            {
                Model.MakeHistory();
        ...
        {
            if ((sender as MenuItem).BindingContext is MonsterSkillViewModel f) await Navigation.PushAsync(InfoPage.Show(new Feature(f.Text, f.Detail)));
        }
