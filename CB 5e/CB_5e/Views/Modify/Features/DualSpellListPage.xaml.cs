...
namespace CB_5e.Views.Modify.Features
{
	...
	public partial class DualSpellListPage : ContentPage
	{
        ...
        public DualSpellListPage(IDualSpellListFeatureEditModel parent)
        {
            Model = parent;
        ...
        {
            if (e.PropertyName == "" || e.PropertyName == null || e.PropertyName == "Spells") Fill();
            OnPropertyChanged(e.PropertyName);
        }

        private void Fill() => Entries.ReplaceRange(Model.Spells.Select(f => new StringViewModel(f)));
        ...
        private async void Entries_ItemSelected(object sender, SelectedItemChangedEventArgs e)
        {
            if (e.SelectedItem is StringViewModel fvm) {
                if (move >= 0)
                {
                    ...
                    {
                        Model.Spells.Insert(target, Model.Spells[move]);
                        if (target < move) move++;
                        Model.Spells.RemoveAt(move);
                        Fill();
        ...
        protected override async void OnAppearing()
        {
            ...
            suggestions = Model.Context.SpellsSimple.Keys.OrderBy(s => s).ToList();
        }
    }
}